...
| F3.1.3a — Server-authoritative decision command | Implementation **PASS** at ADO `6bad066` (full SHA `6bad066d945d49feaf642313ec37467e2658dc3f`), direct child of `2249522`. Nested POST decision route, required `Idempotency-Key`, exact individual principal, live Catalog CAB membership + dedicated `cab.record`, caller-owned txn, rejected lifecycle projection, eligibility `NO_LEDGER_ROUND` safety, PostgreSQL D1–D6, live laptop proof on `CHG-2026-000003` / disposable `CHG-2026-000005`. **Not independently accepted; not CLOSED.** Evidence: [`f3-1-3a-implementation-evidence.md`](./f3-1-3a-implementation-evidence.md). F3.1.3b prompt authoring **NO-GO** until F3.1.3a independent acceptance. |
