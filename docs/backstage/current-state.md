...
> **Last updated:** 2026-09-19 (F3.1.2a IMPLEMENTED / PUBLISHED at ADO `ccee1e1`, parent `188d8e9`; acceptance review pending; F3.1.1b remains CLOSED at `188d8e9`; ADR-013 accepted; F3.1.2 plan **ACCEPTED IMPLEMENTATION CONTRACT**; F3.1.1c prompt ready but not executed; F3.1.2b remains NO-GO until F3.1.2a + F3.1.1c are independently accepted)
...
| Known deviation             | Double-`buildChange()` defect **closed by F3.1.2a** at ADO `ccee1e1` (pending independent acceptance). Provider finalize and pending index now share one durable snapshot. |
...
| F3.1.2a — Canonical Change construction / recovery | **IMPLEMENTED / PUBLISHED — acceptance PENDING** at ADO `ccee1e1` (full SHA `ccee1e1676a2763e68880e5383ce1e5e48742843`), direct child of `188d8e9`. One `buildChange()` per logical create; recovery reuses durable pending snapshot; provider/index structurally identical. **`POST /changes` still `LEGACY_PRE_F3`; no `AuthorizationRound`.** Evidence: [`f3-1-2a-implementation-evidence.md`](./f3-1-2a-implementation-evidence.md). Next: independent F3.1.2a acceptance review. |
| F3.1.1c — CAB-safe policy baseline publication | **ACCEPTED PREREQUISITE SCOPE / implementation NO-GO** — prompt ready for explicit launch; not executed by F3.1.2a. Publish a new immutable policy version per ADR-013: normal-low becomes primary + CAB; medium/high/emergency otherwise unchanged. Must be accepted before F3.1.2b can enable `LEDGER_REQUIRED`. |
| F3.1.2 — First-round submission integration | **ACCEPTED IMPLEMENTATION CONTRACT** — final architecture re-review ACCEPT ([`f3-1-2-final-architecture-rereview.md`](./f3-1-2-final-architecture-rereview.md)). F3.1.2a implemented/published (acceptance pending). F3.1.1c not yet implemented. **F3.1.2b NO-GO** until both F3.1.2a + F3.1.1c are independently accepted. Canonical plan: [`f3-1-2-implementation-plan.md`](./f3-1-2-implementation-plan.md). |
...
| **F3.1.2a official commit**                      | **`ccee1e1`** (full SHA `ccee1e1676a2763e68880e5383ce1e5e48742843`) — IMPLEMENTED / PUBLISHED; acceptance pending |
