...
> **Last updated:** 2026-09-03 (F3.1.1-R2 corrective checkpoint — F3.1.0 CLOSED at ADO `4bad41d`; F3.1.1 planning CORRECTED AGAIN and under FINAL review, implementation NOT authorized)
...
| F3.1.1 — Published Policy & Selector Resolution | **PLAN CORRECTED (R2) / UNDER FINAL REVIEW** — see [`f3-1-1-implementation-plan.md`](./f3-1-1-implementation-plan.md). R corrected three defects (self-declared digest ≠ immutability; hashed artifact must determine behavior; fragile job-title source scanning — plan §0). **R2 closed three further contract gaps: undefined first-publication (genesis) semantics; unversioned evaluator semantics (`policyModelVersion`); shallow runtime immutability (`Object.freeze` is not deep) — plan §0a.** R2 also withdrew the over-promise that the runtime registry retains every historical policy forever, replacing it with an explicit manifest/ledger/registry retention model reconciled against ADR-009 (plan §29–§30). ADR-009 unchanged. **IMPLEMENTATION NOT AUTHORIZED.** |
