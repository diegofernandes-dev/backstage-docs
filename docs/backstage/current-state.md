...
> **Last updated:** 2026-09-19 (F3.1.1b CLOSED / ACCEPTED IMPLEMENTED BASELINE at ADO `188d8e9`; F3.1.2 plan revised after architecture REJECT and is **READY_FOR_REREVIEW** at docs `6284195`; F3.1.2 implementation remains NO-GO)
...
| F3.1.2 — First-round submission integration | **Revised plan READY_FOR_REREVIEW / implementation NO-GO** — [`f3-1-2-implementation-plan.md`](./f3-1-2-implementation-plan.md) now embodies the REJECT review decisions: repository explicit mode-mismatch remains `CONFLICT` while stored-mode-wins is service orchestration; caller-owned `trx` is mandatory for ledger/audit/finalize/complete (and DevelopmentProvider); `requirementId = requirementRole` with publication-time uniqueness; rollback to pre-F3.1.2 with pending ledger reservations is a mandatory runbook correctness gate. Fresh independent re-review required before any implementation prompt. |
