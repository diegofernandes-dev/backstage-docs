...
> **Last updated:** 2026-09-19 (F3.1.2a **CLOSED / ACCEPTED IMPLEMENTED BASELINE** at ADO `ccee1e1`, parent `188d8e9`; F3.1.1b remains CLOSED at `188d8e9`; ADR-013 accepted; F3.1.2 plan **ACCEPTED IMPLEMENTATION CONTRACT**; F3.1.1c prompt ready but not executed — requires separate explicit launch; F3.1.2b remains NO-GO until F3.1.1c is implemented and independently accepted)
...
| F3.1.1c — CAB-safe policy baseline publication | **ACCEPTED PREREQUISITE SCOPE / implementation NO-GO** — prompt ready for explicit launch; not executed. Publish a new immutable policy version per ADR-013: normal-low becomes primary + CAB; medium/high/emergency otherwise unchanged. Must be accepted before F3.1.2b can enable `LEDGER_REQUIRED`. |
| F3.1.2 — First-round submission integration | **ACCEPTED IMPLEMENTATION CONTRACT** — final architecture re-review ACCEPT ([`f3-1-2-final-architecture-rereview.md`](./f3-1-2-final-architecture-rereview.md)). F3.1.2a accepted. F3.1.1c not yet implemented. **F3.1.2b NO-GO** until F3.1.1c is implemented and independently accepted. Canonical plan: [`f3-1-2-implementation-plan.md`](./f3-1-2-implementation-plan.md). |
