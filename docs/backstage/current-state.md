...
> **Last updated:** 2026-09-20 (F3.1.2 **CLOSED / ACCEPTED IMPLEMENTED SUBMISSION BASELINE** at ADO `2249522`; non-production `LEDGER_REQUIRED` activation **ACCEPT**; operator-laptop demo target **READY**; final local overlay `LEDGER_REQUIRED`, committed default `LEGACY_PRE_F3`; F3.1.3 planning **READY_FOR_REVIEW** at [`f3-1-3-implementation-plan.md`](./f3-1-3-implementation-plan.md) after independent source re-verification at docs `7725217` / ADO `2249522`; F3.1.3 implementation remains **NO-GO**; F3.1.4/F3.2 **NO-GO**; production cutover unauthorized)
...
| F3.1.3 — Decision command and new-round semantics | Planning **READY_FOR_REVIEW** ([`f3-1-3-implementation-plan.md`](./f3-1-3-implementation-plan.md)) against ADO `2249522` and accepted laptop `LEDGER_REQUIRED` demo target; independently re-verified at docs `7725217`. Recommended slices F3.1.3a (decision command) + F3.1.3b (resubmission); `Migration required: NO`. Ledger **reads** must become trx-aware (source gap, no DDL). Implementation **NO-GO** pending independent review + separate implementation prompt. |
