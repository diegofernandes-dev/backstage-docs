...
> **Last updated:** 2026-09-07 (F3.1.1b IMPLEMENTED / PUBLISHED at ADO `188d8e9`, pending architecture implementation review; F3.1.2+ remains NOT authorized)
...
| F3.1.1b — Selector bundle, resolver & selector publication integrity | **IMPLEMENTED / PUBLISHED** at ADO `188d8e9` (full SHA `188d8e9cc43423f3644b3cacfb9849257838a583`), direct child of `d3c0751`, on `feat/ado-repo-governance`. `SelectorBundle`/`SelectorEntry` domain, `readAuthorizationConfig` over a mandatory `changeManagement.authorization` block, `createSelectorBundle` (duplicate-key rejection, `sha256Canonical(bundle.selectors)` content digest, manifest agreement, `deepFreezeSerializable`), active-pair-only startup validation (coverage, `individual⇔user` / `authority`/`cab` ⇔ `authority`, generic `separationOfDutyKey`-based emergency A/B narrowing, ref-syntax/kind), and a fail-closed Catalog principal resolver using `auth.getOwnServiceCredentials()` — no cache, no member expansion, no address or human-readable-name fallback. One `selector-bundle` entry appended to the existing `published-manifest.json` and validated as an **ordinary append** against the trusted F3.1.1a baseline through the **unchanged** validator; the genesis path was not used. Startup validation, Catalog resolution and authority non-expansion were proven against a real running Backstage/Catalog. **`POST /changes` remains unwired; no `AuthorizationRound` is created. Pending architecture implementation review** — see [`f3-1-1b-implementation-evidence.md`](./f3-1-1b-implementation-evidence.md). |
