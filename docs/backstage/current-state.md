...
> **Last updated:** 2026-09-20 (F3.1.2 **CLOSED / ACCEPTED IMPLEMENTED SUBMISSION BASELINE** at ADO `2249522`; non-production `LEDGER_REQUIRED` activation **ACCEPT**; operator-laptop demo target **READY**; final local overlay `LEDGER_REQUIRED`, committed default `LEGACY_PRE_F3`; F3.1.3 plan architecture review **REJECT** preserved; F3.1.3 plan revision **READY_FOR_REREVIEW** via [ADR-014](../adr/ADR-014-change-resubmission-authority-and-identity-boundary.md); F3.1.3 implementation remains **NO-GO**; F3.1.4/F3.2 **NO-GO**; production cutover unauthorized)
...
| F3.1.3 — Decision command and new-round semantics | Plan architecture review **REJECT** preserved ([`f3-1-3-plan-architecture-review.md`](./f3-1-3-plan-architecture-review.md)). Narrow 3b revision **READY_FOR_REREVIEW** ([`f3-1-3-implementation-plan.md`](./f3-1-3-implementation-plan.md), [ADR-014](../adr/ADR-014-change-resubmission-authority-and-identity-boundary.md)). F3.1.3a decision-command contracts unchanged. G13/G14 closed in the revised plan by dedicated `change.resubmit` + requester-or-immutable-owner proof, and frozen `targetRef`/`ownerRef`/`systemRef`/`requestedBy`/`createdAt`. `Migration required: NO`. Implementation **NO-GO** until the revised plan is independently ACCEPTed. |
