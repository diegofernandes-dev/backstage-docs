...
>
> Historical visual references remain archived in `diegofernandes-dev/poc-teams-approval` until intentionally re-captured/migrated. The textual contract in this repository is normative.
...
**F3 architecture note:** the three steps and “gestor” wording above remain the implemented informational baseline, not canonical policy. Accepted ADR-009 defines versioned effective requirements resolved to provider-neutral principals. A future authorization UI must render those requirements and must not hardcode a manager, corporate title, CAB sequence, DevOps approval, or Teams channel. No UI change was implemented by F3.0.1.
...
After a successful `POST /changes`, the main column swaps to a success card (no route change, no navigation away from `/gmud/new`) offering:
...
The user is never required to remember or copy the `changeId` to find the GMUD again.
...
Historical capture notes and screenshots remain archived in the legacy POC repository until intentionally re-captured or migrated. The textual contract above is authoritative.
